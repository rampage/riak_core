...
%%
%% 4) The new functions gen_server3:pcall/3, pcall/4, and pcast/3
...
process_next_msg(Parent, Name, State, Mod, Time, TimeoutState, Queue,
                 Debug, Hib) ->
    case priority_queue2:out(Queue) of
        {{value, Msg}, Queue1} ->
...
in({'$gen_pcast', {Priority, Msg}}, Queue) ->
    priority_queue2:in({'$gen_cast', Msg}, Priority, Queue);
in({'$gen_pcall', From, {Priority, Msg}}, Queue) ->
    priority_queue2:in({'$gen_call', From, Msg}, Priority, Queue);
in(Input, Queue) ->
    priority_queue2:in(Input, Queue).
...
format_status(Opt, StatusData) ->
    ...
    [{header, Header},
     {data, [{"Status", SysState},
	     {"Parent", Parent},
	     {"Logged events", Log},
             {"Queued messages", priority_queue2:to_list(Queue)}]} |
     Specfic1].
